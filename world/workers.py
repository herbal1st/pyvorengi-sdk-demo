...
def worker_mesh_chunk(
    ...
    # 1. Fetch XY-padded volume depending on world generation mode
    ...
    # 2. Construct the final Meshing Volume (Shape: 26, 18, 18)
    ...
    # 3. Inject Halo Data into the Z-interior
    padded_vol[1:-1, :, :] = gen_padded

    # 4. Inject Core Chunk Data (Player modifications/Clean data)
...
def _get_static_padded_chunk(cx: int, cy: int) -> NDArray[np.uint8]:
    ...
    # Map the valid overlap region into the zero-allocated padded grid
